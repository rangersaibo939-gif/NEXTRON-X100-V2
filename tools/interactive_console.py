...
def make_interactive(project_root: str) -> Path:
    ...
    text = text.replace('    var lastAction by remember { mutableStateOf("") }\n', '    var lastAction by remember { mutableStateOf("") }\n    var taskRequest by remember { mutableStateOf("") }\n')
    text = text.replace(
    ...
    replacements = {
        ...
        'Button(onClick = { lastAction = "Build APK"; actionCount++ }) { Text("Build APK") }': 'Button(onClick = { lastAction = "Build requested — use the GitHub Actions APK pipeline"; actionCount++ }) { Text("Build APK") }',
        'Button(onClick = { lastAction = "Download APK"; actionCount++ }) { Text("Download APK") }': 'Button(onClick = { lastAction = "APK artifact is available from the latest GitHub Actions run"; actionCount++ }) { Text("Download APK") }',
    }
