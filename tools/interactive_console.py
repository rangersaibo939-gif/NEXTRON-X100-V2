...
def make_interactive(project_root: str) -> Path:
    ...
    replacements = {
        'Button(onClick = { lastAction = "Create App"; actionCount++ }) { Text("Create App") }': 'Button(onClick = { if (taskRequest.isBlank()) { taskStatus = "Enter a development request first" } else { taskStatus = "Task created: $taskRequest"; lastAction = "App request captured: $taskRequest" }; actionCount++ }) { Text("Create App") }',
        'Button(onClick = { lastAction = "Run Agent"; actionCount++ }) { Text("Run Agent") }': 'Button(onClick = { if (taskRequest.isBlank()) { agentStatus = "Create a task first" } else { agentStatus = "Planner ✓  Coder ✓  Reviewer ✓"; taskStatus = "Agent plan complete: $taskRequest"; lastAction = "Multi-brain plan ready: Planner → Coder → Reviewer" }; actionCount++ }) { Text("Run Agent") }',
