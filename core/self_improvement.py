...
class SelfImprovementLoop:
    """Inspect NEXTRON capabilities, choose the next gap, and run a multi-brain plan."""
    ...
    def create_task(self, capability: str, description: str | None = None) -> DevelopmentTask:
        ...
        task = DevelopmentTask(f"Implement {name}", name, description or f"Add and verify NEXTRON capability: {name}.")
    ...
    def run_agent(self, task: DevelopmentTask) -> SelfImprovementResult:
        ...
        return SelfImprovementResult(tuple(sorted(self.capabilities)), self.inspect(), task, result.consensus, "planned")
